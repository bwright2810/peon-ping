  ...
  (
    ...
    if [ "$ELAPSED" -gt 86400 ]; then
      ...
      REMOTE_VERSION=$(curl -fsSL --connect-timeout 3 --max-time 5 \
        "https://raw.githubusercontent.com/bwright2810/peon-ping/main/VERSION" 2>/dev/null | tr -d '[:space:]')
...
if [ "$EVENT" = "SessionStart" ] && [ -f "$PEON_DIR/.update_available" ]; then
  ...
  if [ -n "$NEW_VER" ]; then
    echo "peon-ping update available: ${CUR_VER:-?} → $NEW_VER — run: curl -fsSL https://raw.githubusercontent.com/bwright2810/peon-ping/main/install.sh | bash" >&2
  fi
fi
